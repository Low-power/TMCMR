...
/*
	private static void loadOldChunkData( McRegionChunkData cd, byte[] block_types, int dx, int dy, int dwidth ) {
		byte[] blockData = cd.blockData;
		for( int z=0; z<16; z++ ) {
			for( int x=0; x<16; x++ ) {
				for( int y=127; y>=0; y-- ) {
					int blockIdx = y + z*128 + x*16*128;
					//System.err.printf("loadOldChunkData: blockIdx = %d\n", blockIdx);
					byte blockType = blockData[blockIdx];
					if( blockType == 0 ) continue;
					int didx = (dy+z)*dwidth+dx+x;
					block_types[didx] = blockType;
					break;
				}
			}
		}
...
*/
	...
	protected void preRender( RegionFile rf, int[] colors, short[] heights ) {
		...
		if(rf.is_anvil_format()) {
			...
			usedSections = null;
			biomeIds = null;
			//old_block_ids = new byte[512*512];
			...
			for( int cx=0; cx<32; ++cx ) {				
				...
				try {
					...
					if(rf.is_anvil_format()) {
						...
						//loadOldChunkData(McRegionChunkData.fromTag(levelTag), old_block_ids, cx*16, cz*16, 512);
						loadOldChunkData(levelTag, old_block_ids, old_block_data);
					}
					timer.regionLoading += getInterval();

					if(rf.is_anvil_format()) for( int s=0; s<maxSectionCount; ++s ) {
						if( usedSections[s] ) {
							++timer.sectionCount;
						}
					}
